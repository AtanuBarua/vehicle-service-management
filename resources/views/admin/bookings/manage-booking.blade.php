            ...
            <div class="table-wrapper">
                ...
                <table id="datatable1" class="table display responsive nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="width: 5%">{{$booking->service_name}}</td>
                            <td style="width: 5%">{{$booking->region_name}} - {{$booking->city_name}}</td>
                            ...
                            <td style="width: 5%"><a href="{{route('manage-technicians')}}">{{$booking->technician_name}}</a></td>
                            <td style="width: 5%">
                                
                                @if($booking->status==0)
                                <a onclick="return confirm('Are you sure to confirm?');" class="btn btn-warning btn-sm" href="{{route('confirm-booking',['id'=>$booking->id])}}"><i title="Confirm Booking" class="fa fa-check-square-o" aria-hidden="true"></i>
                                </a>
                                <a onclick="return confirm('Are you sure to cancel?');" class="btn btn-danger btn-sm" href="{{route('cancel-booking',['id'=>$booking->id])}}" class="ml-2"><i title="Cancel Booking" class="fa fa-ban" aria-hidden="true"></i></a>
                                @elseif($booking->status==3)
                                <a onclick="return confirm('Process again?');" class="btn btn-danger btn-sm" href="{{route('process-again',['id'=>$booking->id])}}"><i title="Process Again" class="fa fa-check-circle" aria-hidden="true"></i>
                                </a>
                                <a onclick="return confirm('Delivery done?');" class="btn btn-success btn-sm" href="{{route('delivered-booking',['id'=>$booking->id])}}"><i title="Done Delivery" class="fa fa-check-circle" aria-hidden="true"></i>
                                </a>
